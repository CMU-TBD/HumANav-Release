...
def plot_topview(ax,
    ...
    # Plot the camera
    ax.plot(camera_pos_13[0],
            camera_pos_13[1],
            'bo',
            markersize=10,
            label='Camera')
    ax.quiver(camera_pos_13[0], camera_pos_13[1], np.cos(camera_pos_13[2]),
              np.sin(camera_pos_13[2]))
    ...
    for i, human in enumerate(humans):
        ...
        human.get_trajectory().render(ax,
                                      freq=1,
                                      color=color,
                                      plot_quiver=False)
        if (i == 0):
            # Only add label on the first humans
            ax.plot(human_pos_2[0],
                    human_pos_2[1],
                    'ro',
                    markersize=10,
                    label='Human')
            ax.plot(human_goal_2[0],
                    human_goal_2[1],
                    markerfacecolor="#FF7C00",
                    marker='o',
                    markersize=10,
                    label='Goal')
        else:
            ...
            ax.plot(human_goal_2[0],
                    ...
                    markersize=10)
        if (plot_quiver):
            ax.quiver(human_pos_2[0],
                      human_pos_2[1],
                      np.cos(human_heading),
                      np.sin(human_heading),
                      scale=2,
                      scale_units='inches')
...
def plot_images(p, rgb_image_1mk3, depth_image_1mk1, environment, room_center,
    ...
    traversible = environment["traversibles"][
        0]  # Obstacles/building traversible
    ...
    outer_zoom = min(traversible.shape[0],
                     traversible.shape[1]) * map_scale  # to keep square plot
    ax = fig.add_subplot(1, 4, 2)
    ax.set_xlim(0., outer_zoom)
    ax.set_ylim(0., outer_zoom)
    plot_topview(ax, extent, traversible, human_traversible, camera_pos_13,
                 humans)
    ax.legend()
    ax.set_xticks([])
    ax.set_yticks([])
    ...
    if (not os.path.exists(full_file_name)):
        print('\033[31m', "Failed to find:", full_file_name, '\033[33m',
...
def test_socnav(num_humans):
    ...
    r = HumANavRendererMulti.get_renderer(
        p)  #get the renderer from the camera p
    ...
    camera_pos_13 = np.array([[9., 22., -np.pi / 4]])
    ...
    for i in range(num_humans):
        ...
        new_human_i = Human.generate_random_human_from_environment(
            Human, surreal_data, environment, room_center, radius=5)
        ...
        environment["traversibles"] = (traversible, r.get_human_traversible()
    ...
    for i in range(num_cameras):
        ...
        plot_images(p, rgb_image_1mk3, depth_image_1mk1, environment,
                    room_center, camera_pos_13[i], human_list,
                    "example1_v" + str(i) + ".png")
