...
def test_planner():
    ...
    goal_pos_n11 = tf.constant([[[12., 18.75]]], dtype=tf.float32) # Goal position (must be 1x1x2 array)
    goal_heading_n11 = tf.constant([[[-np.pi/2.]]], dtype=tf.float32)
    # Start states and initial speeds
    start_pos_n11 = tf.constant([[[27.75, 25]]], dtype=tf.float32) # Goal position (must be 1x1x2 array)
    start_heading_n11 = tf.constant([[[np.pi]]], dtype=tf.float32)
