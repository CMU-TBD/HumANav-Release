...
def test_dubins_v1(visualize=False):
    ...
    x1, x2, x3, x4 = (state_nk3[0, 0], state_nk3[0, 1],
                      state_nk3[0, 2], state_nk3[0, 3])
...
def test_custom_dubins_v1(plot=False):
    ...
    ctrl_1k2 = np.array(np.concatenate(
        [v_1k, w_1k], axis=1)[None], dtype=np.float32)
...
def test_dubins_v3():
    ...
    assert(np.allclose(x4, [np.cos(.01) * .1 * .2 + .01,
                            np.sin(.01) * .1 * .2, .03, .3, .3], atol=1e-4))
