...
def test_coordinate_transform():
    ...
    ref_config_heading_nk1 = np.repeat(
        ref_config_heading_111, repeats=n, axis=0)
    ref_config = SystemConfig(dt=dt, n=n, k=1,
                              position_nk2=ref_config_pos_n12,
...
def visualize_coordinate_transform():
    ...
    dubins_car.to_world_coordinates(
        start_config_global, traj_egocentric, traj_world)
...
def test_lqr_feedback_coordinate_transform():
    ...
    start_pos_n13 = np.array(
        np.array([[[0.0, 0.0, 0.0]]], dtype=np.float32))
    speed_nk1 = np.ones((n, k - 1, 1), dtype=np.float32) * 2.0
    angular_speed_nk1 = np.linspace(
        1.5, 1.3, k - 1, dtype=np.float32)[None, :, None]
    ...
    start_pos_n13 = np.array(
        np.array([[[1.0, 1.0, np.pi / 2.]]], dtype=np.float32))
