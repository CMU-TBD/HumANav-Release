...
def test_voxel_interpolation():
    ...
    x = np.linspace(grid_origin[0], grid_origin[0] +
                    scale * (grid_size[0] - 1), grid_size[0])
    y = np.linspace(grid_origin[1], grid_origin[1] +
                    scale * (grid_size[1] - 1), grid_size[1])
    ...
    voxel_map = VoxelMap(scale=scale,
                         origin_2=np.array(
                             grid_origin / scale, dtype=np.float32),
    ...
    test_positions = np.array([[[5.02, 6.01], [5.67, 7.73], [6.93, 6.93]], [[9.1, 7.2], [7.889, 8.22], [7.1, 8.1]]],
                              dtype=np.float32)

    # Get the interpolated output of the voxel map
    interpolated_values = voxel_map.compute_voxel_function(
        test_positions, invalid_value=-1.)
    ...
    expected_interpolated_values = np.reshape(
        expected_interpolated_values, [-1])
    expected_interpolated_values[3] = -1.

    # Scipy Interpolated values
    f_scipy = interpolate.RectBivariateSpline(y, x, zv, kx=1, ky=1)
    scipy_interpolated_values = f_scipy.ev(np.reshape(test_positions[:, :, 1], [-1]),
                                           np.reshape(test_positions[:, :, 0], [-1]))
    scipy_interpolated_values[3] = -1.

    assert np.sum(abs(expected_interpolated_values -
                      interpolated_values) <= 0.01) == 6
    assert np.sum(abs(scipy_interpolated_values -
                      interpolated_values) <= 0.01) == 6
