...
class Spline3rdOrder(Spline):
    ...
    def fit(self, start_config, goal_config, final_times_n1=None, factors=None):
        ...
        if factors is None:  # Compute them heuristically
            factor1_n1 = start_config.speed_nk1()[:, :, 0] + \
                np.linalg.norm(goal_config.position_nk2() -
                               start_config.position_nk2(), axis=2)
        ...
        self.p_coeffs_n14 = np.stack([a3_n1, b3_n1, c3_n1, 0.0 * c3_n1],
                                     axis=2)
    ...
    def _eval_spline(self, ts_nk, calculate_speeds=True):
        ...
        if calculate_speeds:
            ...
            ps_ddot_n4k = np.stack([6.0 * ps_nk, 2.0 * np.ones_like(ps_nk),
                                    np.zeros_like(ps_nk),
                                    np.zeros_like(ps_nk)], axis=1)

            ps_dot_nk = np.squeeze(
                np.matmul(p_coeffs_n14, ts_dot_n4k), axis=1)

            xs_ddot_nk = np.squeeze(
                np.matmul(x_coeffs_n14, ps_ddot_n4k), axis=1)
            ys_ddot_nk = np.squeeze(
                np.matmul(y_coeffs_n14, ps_ddot_n4k), axis=1)
    ...
    def ensure_goals_valid(start_x, start_y, goal_x_nk1, goal_y_nk1, goal_theta_nk1, epsilon):
        ...
        assert((goal_theta_nk1 >= -np.pi / 2.).all()
               and (goal_theta_nk1 <= np.pi / 2.).all())
        norms = np.linalg.norm(np.concatenate(
            [goal_x_nk1 - start_x, goal_y_nk1 - start_y], axis=2), axis=2)
        invalid_idxs = (norms == 0.0)
        goal_x_nk1[invalid_idxs] += epsilon
        goal_y_nk1[invalid_idxs] += np.sign(
            np.sin(goal_theta_nk1[invalid_idxs])) * epsilon
