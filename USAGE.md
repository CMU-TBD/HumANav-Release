...
- We do provide the option under `[agent_params]` in [`user_params.ini`](params/user_params.ini) to set `collision_cooldown_amnt` to any positive integer representing the number of simulator timesteps that the agents will halt motion after a collision with the robot. This feature applies to both Auto-agents as well as Prerecorded-agents. 
