...
class Episode():
    def __init__(self, name, environment, agents, t_budget, r_start, r_goal):
        self.name = name
...
class Joystick():
    ...
    def create_message(self, joystick_status: bool, v_cmds: list, a_cmds: list,
    ...
    def random_inputs(self, amnt: int, pr: int = 100):
        ...
        for _ in range(amnt):
            ...
            num_cmds_sent += 1
        self.robot_input(lin_vels, ang_vels, request_world=True)
    ...
    def await_episodes(self):
        ...
        while self.episode_names is None:
            sleep(0.01)
    ...
    def ensure_ready_for_episode(self):
        assert(len(self.episode_names) > 0)
        assert(self.current_ep is not None)
    ...
    def update_knowledge_from_episode(self, current_world, init_ep: bool = False):
        ...
        if(init_ep):
            ...
            print("Created episode {} simulator".format(name))
